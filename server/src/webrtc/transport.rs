...
impl SharedPacket {
    pub fn new(payload: Vec<u8>, marker: bool) -> Self {
        Self { payload: Bytes::from(payload), marker }
    }
}
...
impl IceServer {
    ...
    pub fn turn(url: impl Into<String>, username: impl Into<String>, credential: impl Into<String>) -> Self {
        Self { urls: vec![url.into()], username: Some(username.into()), credential: Some(credential.into()) }
    }
...
fn default_ice_trickle() -> bool { true }
fn default_ice_timeout() -> u64 { 30 }
fn default_max_peers() -> usize { 10 }
fn default_video_codec() -> VideoCodecType { VideoCodecType::H264 }
fn default_payload_type() -> u8 { 96 }
fn default_clock_rate() -> u32 { 90000 }
...
impl WebRtcConfig {
    pub fn builder() -> WebRtcConfigBuilder { WebRtcConfigBuilder::default() }
    ...
    fn to_rtc_configuration(&self) -> RTCConfiguration {
        RTCConfiguration {
            ice_servers: self.ice_servers.iter().map(|s| s.to_rtc_ice_server()).collect(),
            ..Default::default()
        }
    }
}
...
impl WebRtcConfigBuilder {
    ...
    pub fn add_turn_server(mut self, url: impl Into<String>, username: impl Into<String>, credential: impl Into<String>) -> Self {
    ...
    pub fn build(self) -> Result<WebRtcConfig> {
        Ok(WebRtcConfig {
            ice_servers: if self.ice_servers.is_empty() { vec![IceServer::stun("stun:stun.l.google.com:19302")] } else { self.ice_servers },
...
pub enum ConnectionState { Connecting, Connected, Disconnected, Failed, Closed }
...
impl PeerConnectionHandle {
    pub fn state(&self) -> ConnectionState { self.peer_connection.connection_state().into() }
    pub fn is_active(&self) -> bool { matches!(self.state(), ConnectionState::Connected | ConnectionState::Connecting) }
    pub fn id(&self) -> &str { &self.connection_id }

    pub async fn send_video_frame(&mut self, frame: &VideoFrame) -> Result<()> {
        if !self.is_active() { return Err(Error::Transport("Connection not active".into())); }
        let packets = match frame.frame_type {
            FrameType::H264 => packetize_h264_optimized(&frame.data),
            _ => return Err(Error::Transport(format!("Unsupported codec: {:?}", frame.frame_type))),
        };
        for shared in packets {
            let mut header = webrtc::rtp::header::Header {
                version: 2, marker: shared.marker, payload_type: 96,
                sequence_number: self.sequence_number, timestamp: self.timestamp, ssrc: self.ssrc,
                ..Default::default()
            };
            if let Some(id) = self.playout_delay_id { header.set_extension(id, Bytes::from_static(&[0, 0, 0])).ok(); }
            let packet = RtpPacket { header, payload: shared.payload };
            self.video_track.write_rtp(&packet).await.map_err(|e| Error::Transport(e.to_string()))?;
    ...
    pub async fn add_ice_candidate(&self, candidate: IceCandidate) -> Result<()> {
        self.peer_connection.add_ice_candidate(candidate.to_rtc_candidate_init()).await.map_err(|e| Error::Transport(e.to_string()))
    }

    pub async fn next_ice_candidate(&mut self) -> Option<IceCandidate> { self.ice_candidate_rx.next().await }
}
...
pub struct H264ParameterSets { pub sps: Bytes, pub pps: Bytes }

#[derive(Debug, Clone)]
pub enum PeerEvent {
    Connected { connection_id: String, source_id: String },
    Disconnected { connection_id: String, source_id: String },
    IceCandidate { connection_id: String, candidate: IceCandidate },
}
...
impl PeerConnectionManager {
    pub async fn new(config: WebRtcConfig) -> Result<Self> {
        ...
        Ok(Self {
            config, api, connections: Arc::new(RwLock::new(HashMap::new())),
            source_connections: Arc::new(RwLock::new(HashMap::new())),
            event_tx, h264_parameter_sets: Arc::new(RwLock::new(HashMap::new())),
        })
    }

    pub fn update_h264_parameter_sets(&self, source_id: &str, sps: Bytes, pps: Bytes) {
        if !sps.is_empty() && !pps.is_empty() {
            self.h264_parameter_sets.write().insert(source_id.to_string(), H264ParameterSets { sps, pps });
        }
    }

    pub async fn handle_offer(&self, source_id: &str, offer: SdpOffer) -> Result<(SdpAnswer, PeerConnectionHandle)> {
    ...
    fn h264_fmtp_line(&self, source_id: &str) -> Option<String> {
        ...
        let sprop = format!("{},{}", BASE64_STANDARD.encode(sps), BASE64_STANDARD.encode(pps));
        let mut parts = vec!["packetization-mode=1".to_string()];
        if let Some(profile) = h264_profile_level_id(sps) { parts.push(format!("profile-level-id={}", profile)); }
        parts.push(format!("sprop-parameter-sets={}", sprop));
        Some(parts.join(";"))
    }

    pub fn connection_count_for_source(&self, source_id: &str) -> usize { self.source_connections.read().get(source_id).map(|v| v.len()).unwrap_or(0) }
    pub fn total_connection_count(&self) -> usize { self.connections.read().len() }
    ...
    pub async fn broadcast_frame(&self, connection_ids: Vec<String>, timestamp: u32, packets: &[SharedPacket]) -> Result<usize> {
        if connection_ids.is_empty() { return Ok(0); }

        let mut sent_count = 0;
        for conn_id in connection_ids {
            if self.send_optimized_packets(&conn_id, timestamp, packets).await? {
                sent_count += 1;
            }
        }
        Ok(sent_count)
    ...
    async fn send_optimized_packets(&self, connection_id: &str, timestamp: u32, packets: &[SharedPacket]) -> Result<bool> {
        let (track, mut sequence_number, ssrc, playout_delay_id) = {
            ...
            (Arc::clone(&conn.video_track), conn.sequence_number, conn.ssrc, conn.playout_delay_id)
        };
        for shared in packets {
            let mut header = webrtc::rtp::header::Header { version: 2, marker: shared.marker, payload_type: 96, sequence_number, timestamp, ssrc, ..Default::default() };
            if let Some(id) = playout_delay_id { header.set_extension(id, Bytes::from_static(&[0, 0, 0])).ok(); }
            let packet = RtpPacket { header, payload: shared.payload.clone() };
            if track.write_rtp(&packet).await.is_ok() { sequence_number = sequence_number.wrapping_add(1); }
        }
...
fn strip_annex_b(data: &[u8]) -> &[u8] {
    if data.starts_with(&[0, 0, 0, 1]) { &data[4..] } else if data.starts_with(&[0, 0, 1]) { &data[3..] } else { data }
}
